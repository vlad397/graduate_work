...
@routes.route('/grant_permission', methods=['POST'])
@marshal_with(ResponseSchema())
@use_kwargs(GrantPermissionSchema())
@doc(description='Method for grant permission', tags=['permission'], params=params_auth_default)
@jwt_required()
@admin_required()
@logout_session()
def grant_permission(**kwargs):
    ...
    return {MESSAGE: 'Granted permission'}, HTTPStatus.OK
